...
var require_lib = __commonJS({
  "node_modules/node-fetch/node_modules/whatwg-url/node_modules/webidl-conversions/lib/index.js"(exports2, module2) {
    "use strict";
    var conversions = {};
...
var require_mappingTable = __commonJS({
  "node_modules/node-fetch/node_modules/whatwg-url/node_modules/tr46/lib/mappingTable.json"(exports2, module2) {
...
var require_tr46 = __commonJS({
  "node_modules/node-fetch/node_modules/whatwg-url/node_modules/tr46/index.js"(exports2, module2) {
    "use strict";
...
var VERSION, RETRY_STATUS_CODES, ENDPOINT, configuredFetch, MistralAPIError, MistralClient, client_default;
var init_client = __esm({
  "node_modules/@mistralai/mistralai/src/client.js"() {
    ...
    configuredFetch = Promise.resolve(
      globalThis.fetch ?? Promise.resolve().then(() => (init_lib(), lib_exports)).then((m) => m.default)
    );
      ...
      _request = async function(method, path, request, signal) {
        ...
        const options = {
          ...
          signal: combineSignals([
            AbortSignal.timeout(this.timeout * 1e3),
            signal
          ])
        };
...
${await response.text()}`
      ...
      _makeChatCompletionRequest = function(model, messages, tools, temperature, maxTokens, topP, randomSeed, stream, safeMode, safePrompt, toolChoice, responseFormat) {
        ...
        return {
          ...
          tool_choice: toolChoice ?? void 0,
          response_format: responseFormat ?? void 0
        };
      };
      /**
...
// index.ts
var rag_chat_exports = {};
__export(rag_chat_exports, {
...
module.exports = __toCommonJS(rag_chat_exports);
...
var import_env6 = require("@langchain/core/utils/env");
...
function convertMessagesToMistralMessages(messages) {
  ...
  const getTools = (message) => {
    if ((0, import_messages2.isAIMessage)(message) && !!message.tool_calls?.length) {
      return message.tool_calls.map((toolCall) => ({
        ...toolCall,
        id: _convertToolCallIdToMistralCompatible(toolCall.id ?? "")
      })).map(import_openai_tools.convertLangChainToolCallToOpenAI);
    }
  ...
  switch (message.role) {
    case "assistant": {
      ...
      for (const rawToolCall of rawToolCalls) {
        try {
          const parsed = (0, import_openai_tools.parseToolCall)(rawToolCall, { returnId: true });
          toolCalls.push({
            ...parsed,
            id: parsed.id ?? v4_default2().replace(/-/g, "")
          });
        } catch (e) {
          invalidToolCalls.push((0, import_openai_tools.makeInvalidToolCall)(rawToolCall, e.message));
        }
      }
    ...
    default:
      return new import_messages2.HumanMessage(message.content ?? "");
  }
}
function _convertDeltaToMessageChunk(delta, usage) {
  if (!delta.content && !delta.tool_calls) {
    if (usage) {
      return new import_messages2.AIMessageChunk({
        content: "",
        usage_metadata: usage ? {
          input_tokens: usage.prompt_tokens,
          output_tokens: usage.completion_tokens,
  ...
  if (role === "user") {
    return new import_messages2.HumanMessageChunk({ content });
    ...
    return new import_messages2.ToolMessageChunk({
      content,
      additional_kwargs,
      tool_call_id: rawToolCallChunksWithIndex?.[0].id ?? ""
    });
  } else if (role === "function") {
    return new import_messages2.FunctionMessageChunk({
      content,
      additional_kwargs
    });
  } else {
    return new import_messages2.ChatMessageChunk({ content, role });
  }
}
...
var ChatMistralAI = class extends import_chat_models.BaseChatModel {
  ...
  constructor(fields) {
    ...
    const apiKey = fields?.apiKey ?? (0, import_env6.getEnvironmentVariable)("MISTRAL_API_KEY");
    if (!apiKey) {
  ...
  withStructuredOutput(outputSchema, config) {
    ...
    if (method === "jsonMode") {
      ...
      if (isZodSchema(schema)) {
        outputParser = import_output_parsers.StructuredOutputParser.fromZodSchema(schema);
      } else {
        outputParser = new import_output_parsers.JsonOutputParser();
      }
    } else {
      ...
      if (isZodSchema(schema)) {
        ...
        outputParser = new import_openai_tools.JsonOutputKeyToolsParser({
          returnSingle: true,
    ...
    const parserAssign = import_runnables.RunnablePassthrough.assign({
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      parsed: (input, config2) => outputParser.invoke(input.raw, config2)
    });
    const parserNone = import_runnables.RunnablePassthrough.assign({
      parsed: () => null
    });
  ...
  /** @ignore */
  async imports() {
    const { default: MistralClient2 } = await Promise.resolve().then(() => (init_client(), client_exports));
...
var import_env7 = require("@langchain/core/utils/env");
...
var import_env8 = require("@langchain/core/utils/env");
var import_chunk_array2 = require("@langchain/core/utils/chunk_array");
var import_async_caller2 = require("@langchain/core/utils/async_caller");
...
var initializeRedis = () => {
  try {
    return import_redis.Redis.fromEnv();
  } catch {
    return;
  }
};
  ...
  async makeStreamingLLMRequest(prompt, {
    ...
    if (isOpenAIChatLanguageModel(this.model)) {
      ...
      stream = await this.model.stream([
        new import_messages3.HumanMessage(prompt)
      ]);
    }
    const reader = stream.getReader();
    ...
    return traceable(
      async (input2, options2) => {
        try {
          ...
          const { formattedContext: context, metadata } = await this.context._getContext(
            optionsWithDefault,
            input2,
            this.debug
  ...
  async getChatHistory(_optionsWithDefault) {
    return traceable(
      async (optionsWithDefault) => {
        ...
        const formattedHistory = modifiedChatHistory.reverse().map((message) => {
